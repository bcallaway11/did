# did 2.5.1.902
  ...
  * Bug fix (`att_gt()`, `panel = TRUE` with `allow_unbalanced_panel = FALSE`): whether a never-treated group is available is now re-checked *after* the data are coerced to a balanced panel. Balancing drops units whole, so one covariate missing in a single period for every never-treated unit (an ordinary data shape: a variable not collected in one wave for one subgroup) removed the entire never-treated group after the check had already passed. Nothing noticed: under `control_group = "nevertreated"` every ATT(g,t) came back `NA`, and under `"notyettreated"` every cell that needed a never-treated unit did — with no warning naming the cause under `faster_mode = TRUE`, and with a misleading "overlap condition violated" per cell under `faster_mode = FALSE` (there were zero control units, not poor overlap). When balancing removes the last never-treated unit, both code paths now apply the fallback that the package applies when the raw data contain no never-treated group (the last treated cohort serves only as the comparison group, and time periods from its treatment date, net of `anticipation`, onward are dropped), announce it with a warning that says how many units were lost and how to keep them (`allow_unbalanced_panel = TRUE`), and return exactly the estimates obtained by passing the balanced sample — the units observed in every period that remains after the raw-data fallback (all periods when the raw data contain a never-treated group) — directly. Under `"notyettreated"` this is the only situation that warns; the routine fallback on raw data without a never-treated group stays silent, as before. The same re-check covers the comparison cohort created by that fallback being removed by balancing. Under `"notyettreated"` the last cohort's pre-treatment cells that still had a not-yet-treated comparison — previously reported (under `base_period = "varying"`) while all its post-treatment cells were `NA` — are no longer computed, consistent with the rule that this cohort is a comparison group only. Data with never-treated units that survive balancing, `allow_unbalanced_panel = TRUE`, and repeated cross sections are unaffected. The same re-check applies to `conditional_did_pretest()` and to the exported `pre_process_did()`/`pre_process_did2()`, which share this pre-processing.

  * Bug fix (`att_gt()`, same setting): the list of treated cohorts is now reconciled with the balanced sample. When balancing removed every unit of a *treated* cohort (same data shape), `faster_mode = TRUE` — the default — stopped with "Internal error: treated group g not found in cohort_vec. Please report this issue.", and `faster_mode = FALSE` returned `NA` for every cell of that cohort with no explanation. Both paths now drop the cohort from the estimation with a warning naming it, and return the estimates obtained on the balanced sample directly; if balancing removes every treated cohort, `att_gt()` errors with a message that says so, and the no-never-treated error names any cohort that balancing removed. On `faster_mode = FALSE`, an integer `gname` is now stored as double (as `faster_mode = TRUE` always did), so cohort labels in messages are identical across modes and the exported `pre_process_did()` returns double cohort codes as the fast path does (`att_gt()`'s `group` was already double on both paths).

  * `att_gt()` error and warning messages around these situations are more precise: the "No valid groups" error now explains the no-never-treated case (a single treated cohort left, or all others already treated in the first period net of `anticipation`) instead of pointing at a `gname` coding problem that does not exist; data in which every period is at or after the last cohort's treatment date net of `anticipation` (for example a single cohort treated in the first period, or every unit coded 1 in a `gname` meant as a treatment indicator) now get a message naming that cause instead of an unrelated error blaming the balanced-panel coercion or missing data; an estimation sample that is empty after rows with missing or non-finite values are dropped now stops with the missing-data message on both paths (the cohort logic used to run on it and fabricate a "last treated cohort"); the "No never-treated group is available" warning now names the cohort and says that periods *at or after* its treatment date (net of `anticipation`) are dropped, which is what the code does; and a fallback that leaves a single time period stops identically on both paths instead of failing with an internal error on `faster_mode = FALSE`; and when balancing removes every unit (each cohort missing a different period), the error now gives the balanced-panel remedy (`allow_unbalanced_panel = TRUE` or the missing data) instead of suggesting `panel = FALSE` and revisiting `idname`.

  * `aggte()` now says so when `att_gt()` returned no post-treatment cell at all (every cell has t < g — what remains when the periods from the last cohort's treatment date onward were dropped because no never-treated group was available, possibly together with `anticipation`), for every aggregation type; it used to fail with a message blaming `NA` estimates that did not exist.

  * Cohort and period codes in the new messages are printed as written in the data (`500000`, not `5e+05`).

  * A (g,t) cell whose 2x2 estimate is `NaN` (no effective treated or control observations, e.g. all their weights are zero) is now announced with a warning on both paths; it was set to `NA` silently.

  * `att_gt(faster_mode = TRUE)` now warns for a (g,t) cell that has no treated or no control units in either of its two periods ("No units in group g in time period t; the ATT for this cell is set to NA" / "No available control units for group g in time period t; the ATT for this cell is set to NA"), so every `NA` cell is announced on both paths; previously such a cell was set to `NA` with no message (a cell with only one empty period was already reported). The slow path continues to report each empty period separately. With `print_details = TRUE`, `faster_mode = TRUE` now labels each (g,t) cell by its own period; under `base_period = "varying"` it printed the preceding period (off by one).

  * `?att_gt` now documents, under `control_group` and `allow_unbalanced_panel`, what happens when no never-treated units are available and when balancing removes a whole cohort.
...
# did 2.5.1.901
  ...
  * **Behavior change** (`aggte()`): `min_e` is now honored by `type = "simple"` and `type = "group"`, symmetrically with `max_e`, which has always restricted them. Previously `min_e` was silently ignored for these two aggregations, so `aggte(..., type = "simple", min_e = 1)` returned the *unrestricted* overall effect. The defaults (`min_e = -Inf`, `max_e = Inf`) are unchanged, and negative values of `min_e` remain a no-op for these types because only post-treatment cells enter them. A `min_e`/`max_e` window that excludes every post-treatment cell now gives a clear error for `type = "simple"` instead of a generic "no valid estimates" message; for `type = "group"`, a window that leaves *some* cohort with no cell at all (for example `min_e = 1` and a cohort first treated in the last period) errors naming that cohort and pointing at `na.rm = TRUE`, which drops it exactly as it drops all-`NA` cohorts. The window for these two types is also now evaluated in the original units of `tname` (event time `e = t - g`), matching `type = "dynamic"` and the documentation; previously `max_e` counted recoded index steps here, which differed from the documented event time on irregularly spaced panels.

  * `aggte()` now warns that `balance_e` is ignored for `type = "simple"` and `type = "group"` (it only applies to `type = "dynamic"`), paralleling the existing warning for `type = "calendar"`.

  * With only two time periods, `att_gt()` now reports a *pointwise* critical value when `cband = TRUE`, making the returned object self-consistent. Both pre-processors have always overridden `cband` to `FALSE` in this case (uniform bands coincide with pointwise intervals with two periods), but `att_gt()` kept using the user's `cband` when computing the critical value, so `MP$c` was a bootstrap sup-t value while `MP$DIDparams$cband` was `FALSE`, the summary was labelled "Pointwise", and `aggte()` inherited `cband = FALSE`. The override is now announced with a message and applied consistently.

  * `att_gt(faster_mode = FALSE)` now emits the same messages as `faster_mode = TRUE` when `allow_unbalanced_panel = TRUE` is resolved against the data ("You have a balanced panel. Setting allow_unbalanced_panel = FALSE." or "You have an unbalanced panel. Proceeding as such."). The slow path performed the same silent reset without saying so. Relatedly, `faster_mode = FALSE` now strips `idname` from `clustervars` before storing it (as the fast path already did), so both paths carry the same value in the returned `DIDparams` object.

  * Documentation corrections in `?att_gt`, `?aggte`, `?mboot`, `?conditional_did_pretest`, and the "Getting Started" vignette: the effect of listing `idname` in `clustervars` under `panel = FALSE`; `allow_unbalanced_panel` being ignored when `panel = FALSE`; `fix_weights` options being identical only for balanced panels (on unbalanced panels `"base_period"`/`"first_period"` also restrict the sample to units observed in the anchor period, with a warning); what `pl`/`cores` actually parallelize (the multiplier bootstrap only, above 2500 influence-function rows, and not on Windows) and where they are accepted but unused; the scope of `min_e`/`max_e`; the two-period `cband` behavior; the repeated-cross-sections guidance in the vignette; and the time-varying-weights message, which described balanced-panel behavior even when the unbalanced per-observation path would run.
