# did 2.3.1.907
  ...
  * `edid()` no longer silently mixes smoothers when `options(edid_omega_method = "sieve")` is combined with the (default) misspecification-robust SE. The weight-estimation channel is not implemented for the sieve smoother — its influence function would be rebuilt with the kernel — so under `"sieve"` that channel is now disabled with a warning and the plug-in efficient SE is reported for it; `estimation_effect` and `higher_order` (which do not rebuild the conditional covariance) remain active. Previously the sieve weights were silently combined with a kernel weight-channel covariance.
