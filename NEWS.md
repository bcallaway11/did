# did 2.3.1.907
  ...
  * `?edid` documents the small-sample calibration of the standard-error options: `misspec_robust = TRUE` (default, with its bundled `estimation_effect` / `higher_order`) is the finite-sample-calibrated SE (a Monte-Carlo audit finds coverage close to and converging to nominal across all weight schemes and both smoothers), while `misspec_robust = FALSE` is the bare asymptotic efficient-IF SE — valid as n grows but anti-conservative in small samples. No behavior change; documentation only.
