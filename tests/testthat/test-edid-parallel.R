# Guard for the parallel cell-loop lever, options(edid_mc_cores). The (g,t) cells are independent, so the
# forked path (parallel::mclapply, edid-fit.R) MUST be numerically identical to the serial path -- a future
# cache/RNG regression inside a forked worker would otherwise pass CI undetected. Fork-based, so this is a
# no-op on Windows (mc_cores is ignored there) and is skipped.
#
...
test_that("edid_mc_cores > 1 is bit-identical to the serial path (att / se / EIF)", {
  skip_on_cran()
  skip_on_os("windows")          # mclapply forking is unavailable on Windows; mc_cores is ignored there
  data(mpdta, package = "did")
  run <- function(cores) {
    old <- options(edid_mc_cores = cores)
    on.exit(options(old))
    set.seed(7)
    edid(mpdta, yname = "lemp", idname = "countyreal", tname = "year",
         gname = "first.treat", xformla = ~ lpop, aggregate = "none")
  }
  f1 <- run(1L)
  f2 <- run(2L)
