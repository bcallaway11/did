...
test_that("healthy design (all cohorts >= 5): guard inert, byte-identical to the legacy fit", {
  ...
  # estimation_effect = FALSE: the pre-guard package also predates the harmonized
  # default that turns the no-covariate weight-estimation correction ON, so the
  # bit-for-bit contract is the plug-in (estimation_effect off) pipeline.
  expect_no_warning(f5 <- fit_thin(df, omega_cov_shrink = "none",
                                   estimation_effect = FALSE, misspec_robust = FALSE))  # no guard, no legacy warnings
  ...
  # min_pair_units = 2 is equally inert here: identical fit (same plug-in configuration)
...
test_that("min_pair_units = 2 reproduces the pre-guard fit bit-for-bit on the 3-unit design", {
  ...
  # (see the healthy-design test above for the contract). estimation_effect = FALSE:
  # the pre-guard package also predates the harmonized weight-estimation default.
