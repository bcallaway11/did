...
test_that("edid_fit contains all required top-level fields", {
  ...
  fit <- edid(df, "outcome", "unit", "time", "first_treat",
              pt_assumption = "all", aggregate = "all", n_bootstrap = 0L)
...
test_that("print.edid_fit() runs without error", {
  ...
  fit <- edid(df, "outcome", "unit", "time", "first_treat", aggregate = "all")
...
test_that("edid() two-cohort: group ATTs are near true values 1.5 and 2.5 (large sample)", {
  ...
  fit <- edid(df, "outcome", "unit", "time", "first_treat",
              pt_assumption = "post", aggregate = "group", n_bootstrap = 0L)
...
# ============================================================
test_that("edid() with n_bootstrap > 0 returns bootstrap field in edid_fit", {
  df  <- make_panel_1cohort(seed = 42)
  fit <- edid(df, "outcome", "unit", "time", "first_treat",
              pt_assumption = "post", aggregate = "overall",
              n_bootstrap = 50L, bootstrap_weights = "rademacher",
              seed = 42L)
...
# 9.9 Clustered edid()
# ============================================================
test_that("edid() with cluster argument produces finite clustered SEs", {
...
test_that("edid() 95% CI contains 2 for large-n ATT=2 DGP", {
  ...
  fit <- edid(df, "outcome", "unit", "time", "first_treat",
              pt_assumption = "post", aggregate = "overall", n_bootstrap = 0L)
