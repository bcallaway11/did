...
```{r echo=FALSE}
source("~/Dropbox/did/tests/did_sims.R")
reset.sim()
...
We can compare this to what happens using the \code{did} package:

```{r}
# estimate group-group time average treatment effects
did.att.gt <- att_gt("Y", "period", "id", "G", data=data, printdetails=FALSE)
summary(did.att.gt)
...
The \code{did} package contains an additional pre-test for the conditional parallel trends assumption in the \code{conditional_did_pretest} function.
