...
Although this is essentially a standard approach in applied work, there are a number of recent papers that point out potentially severe drawbacks of using the TWFE estimation procedure.  These include: Borusyak and Jaravel (2017), Goodman-Bacon (2019) and de Chaisemartin and D'Haultfoeuille (2020).
...
There are entire papers written about this, see, e.g., Borusyak and Jaravel (2017), Goodman-Bacon (2019) and de Chaisemartin and D'Haultfoeuille (2020).  But here is the short version: in a TWFE regression, units whose treatment status doesn't change over time serve as the comparison group for units whose treatment status does change over time.  With multiple time periods and variation of treatment timing, some of these comparisons are:
...
In our paper, Callaway and Sant'Anna (2020), we propose a number of ways to aggregate group-time average treatment effects.  Here, we will just consider a few important ones that we think applied researchers are most often interested in.  First, consider the average effect of participating in the treatment, separately for each group.  This is given by
...
All of these aggregations are available in the **did** package and examples with real data are available in our [Getting Started with the **did** Package](did-basics.html) vignette. In Callaway and Sant'Anna (2020), we also discuss additional aggregation schemes. We encourage you to take a look!
