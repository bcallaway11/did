...
compute.aggte <- function(MP,
  ...
  if (type == "calendar") {
    ...
    calendar.se.inner <- lapply(calendar.tlist, function(t1) {
      ...
      inf.func.t <- as.numeric(get_agg_inf_func(
        att = att,
        inffunc1 = inffunc1,
        whichones = whicht,
        weights.agg = pgt,
    ...
    # get overall influence function
    calendar.inf.func <- get_agg_inf_func(
      att = calendar.att.t,
    ...
    return(AGGTEobj(
      ...
      inf.function = list(
        calendar.inf.func.t = calendar.inf.func.t,
